...
import { useWorkbookStore, useUIStore } from "../model";
import { formulaEngine } from "@features/formula-engine/workers";
...
export function GridCanvas() {
    ...
    const getCellRef = useRef<
        ((row: number, col: number) => CellData | null) | null
    >(null);
    const [filterDropdown, setFilterDropdown] =
        React.useState<FilterDropdownState | null>(null);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            await formulaEngine.addSheet(
                firstState.activeSheetId,
                firstState.sheets[0].name,
            );
            if (destroyed) return;
            ...
            const getCalculatedValue = (
                ...
                const s = useWorkbookStore.getState();
                return (
                    s.calculatedValues[`${s.activeSheetId}:${row}:${col}`] ??
                    null
                );
            };
            ...
            const onCommit = (
                ...
                // 표 자동 확장 감지
                const tableId = tableManager.checkAutoExpand(
                    s.activeSheetId,
                    row,
                    col,
                    getCell,
            ...
            input.setCreateTableCallback(() => {
                ...
                tableManager
                    .createTable(s.activeSheetId, r1, c1, r2, c2, getCell)
                    .catch((err) =>
                        console.error(
                            "[TableManager] createTable failed:",
                            err,
                        ),
                    );
            });
            ...
            const unsubFormula = formulaEngine.onChanged((changed) => {
                ...
                for (const c of changed) {
                    ...
                    next[key] =
                        c.value.t === "nil" ? null : (c.value.v ?? null);
                }
            ...
            const unsubFilter = filterManager.subscribe(() => {
                const s = useWorkbookStore.getState();
                viewport.setHiddenRows(
                    filterManager.hiddenRows.get(s.activeSheetId) ?? new Set(),
                );
            });
            ...
            const unsubWorkbook = useWorkbookStore.subscribe((state, prev) => {
                if (state.activeSheetId !== prev.activeSheetId) {
                    ...
                    viewport.setHiddenRows(
                        filterManager.hiddenRows.get(state.activeSheetId) ??
                            new Set(),
                    );
                }
                if (
                    state.cells !== prev.cells ||
                    state.calculatedValues !== prev.calculatedValues
                ) {
                    offscreenLayer.invalidate();
                }
            });
            ...
            const onCanvasClick = (e: MouseEvent) => {
                ...
                for (let r = af.headerRow + 1; r <= af.endRow; r++)
                    dataRows.push(r);
                ...
                setFilterDropdown({
                    ...
                    position: {
                        x: cellPixel.x,
                        y: cellPixel.y + viewport.getRowHeight(row),
                    },
                    uniqueValues: filterManager.getUniqueValues(
                        s.activeSheetId,
                        col,
                        dataRows,
                        getCell,
            ...
            const renderLoop = () => {
                ...
                selectionRenderer.drawSelections(
                    ctx,
                    curSelections,
                    curActiveCell,
                );
