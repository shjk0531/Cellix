...
export class StyleManager {
    ...
    getStyle(sheetId: string, row: number, col: number): CellStyle {
        return { ...DEFAULT_STYLE, ...this.styles.get(`${sheetId}:${row}:${col}`) }
    }

    applyStyle(sheetId: string, ranges: CellRange[], partial: Partial<CellStyle>): void {
    ...
    private _mergeStyle(base: CellStyle, patch: Partial<CellStyle>): CellStyle {
        ...
        for (const k of ['backgroundColor', 'horizontalAlign', 'verticalAlign',
                         'wrapText', 'numberFormat', 'indent', 'locked'] as const) {
            if (k in patch) (result as Record<string, unknown>)[k] = (patch as Record<string, unknown>)[k]
        }
        return result
    }
