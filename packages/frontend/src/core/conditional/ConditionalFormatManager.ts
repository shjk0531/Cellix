...
export class ConditionalFormatManager {
        ...
        for (const entry of applicable) {
            ...
            if (entry.rule.type === "dataBar") {
                if (typeof cellValue === "number") {
                    ...
                    const nums = rangeVals.filter(
                        (v): v is number => typeof v === "number",
                    );
                    dataBarWidth = this._calcDataBar(
                        cellValue,
                        entry.rule,
                        nums,
                    );
    ...
    private _evalRule(
        ...
        switch (rule.type) {
            ...
            case "colorScale": {
                ...
                const nums = rangeValues.filter(
                    (v): v is number => typeof v === "number",
                );
                return {
                    backgroundColor: this._calcColorScale(value, rule, nums),
                };
            }
    ...
    private _matchCellValue(
        value: CellVal,
        rule: CondFmtCellValueRule,
    ): boolean {
        ...
        switch (rule.operator) {
            ...
            case "isNotBlank":
                return value !== null && value !== "";
            case "isDuplicate":
                return false; // TODO: rangeValues 기반 구현 필요
            case "isUnique":
                return false; // TODO: rangeValues 기반 구현 필요
            default:
                return false;
    ...
    private _calcColorScale(
        value: number,
        rule: CondFmtColorScaleRule,
        values: number[],
    ): string {
        ...
        const resolveMin = (): number => {
            switch (rule.minType) {
                ...
                case "percentile":
                    return sorted[
                        Math.floor(
                            ((rule.minValue ?? 0) / 100) * (sorted.length - 1),
                        )
                    ];
            }
        };
        ...
        const resolveMax = (): number => {
            switch (rule.maxType) {
                ...
                case "number":
                    return rule.maxValue ?? sorted[sorted.length - 1];
                case "percent":
                    return (
                        sorted[0] +
                        ((sorted[sorted.length - 1] - sorted[0]) *
        ...
        if (rule.midColor && rule.midType !== undefined) {
            ...
            const tMid = Math.max(
                0,
                Math.min(1, (midVal - minVal) / (maxVal - minVal)),
            );
            if (t <= tMid) {
                return lerpColor(
                    rule.minColor,
    ...
    private _calcDataBar(
        value: number,
        rule: CondFmtDataBarRule,
        values: number[],
    ): number {
