...
export interface FilterCriteria {
    ...
    textOperator?: 'contains' | 'notContains' | 'beginsWith' | 'endsWith' | 'equals' | 'notEquals'
    textValue?: string
    numberOperator?: 'equals' | 'notEquals' | 'greaterThan' | 'lessThan' | 'between' | 'top10'
    numberValue1?: number
    numberValue2?: number
}
...
export class FilterManager {
    ...
    private _matchCriteria(value: CellValue, criteria: FilterCriteria): boolean {
        switch (criteria.type) {
            ...
            case 'text': {
                ...
                switch (criteria.textOperator) {
                    case 'contains': return value.includes(tv)
                    case 'notContains': return !value.includes(tv)
                    case 'beginsWith': return value.startsWith(tv)
                    case 'endsWith': return value.endsWith(tv)
                    case 'equals': return value === tv
                    case 'notEquals': return value !== tv
                    default: return true
                }
            }
            case 'number': {
                ...
                switch (criteria.numberOperator) {
                    case 'equals': return n1 !== undefined && value === n1
                    case 'notEquals': return n1 !== undefined && value !== n1
                    case 'greaterThan': return n1 !== undefined && value > n1
                    case 'lessThan': return n1 !== undefined && value < n1
                    case 'between':
                        return n1 !== undefined && n2 !== undefined && value >= n1 && value <= n2
                    default: return true
                }
            }
            default:
