...
export class ClipboardManager {
    ...
    private _executePaste(mode: PasteMode): void {
        ...
        for (let r = 0; r < rowCount; r++) {
            for (let c = 0; c < colCount; c++) {
                ...
                const after = this._buildAfter(mode, cells[r][c], targetRow, targetCol)
                this._history.execute(this._makeCellCmd(targetRow, targetCol, after))
            }
        }
        ...
        for (let r = 0; r < rows.length; r++) {
            for (let c = 0; c < rows[r].length; c++) {
                ...
                this._history.execute(
                    this._makeCellCmd(activeCell.row + r, activeCell.col + c, after),
                )
            }
        }
    ...
    private _makeCellCmd(row: number, col: number, after: CellData | null): Command {
    ...
    private _serializeTSV(cells: (CellData | null)[][]): string {
        return cells
            .map(row => row.map(cell => this._escapeForTSV(cell)).join('\t'))
            .join('\n')
    }
