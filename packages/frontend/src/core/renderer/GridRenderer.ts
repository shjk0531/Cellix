    ...
    draw(ctx: CanvasRenderingContext2D): void {
        ...
        for (const { row, y, h } of rows) {
            for (const { col, x, w } of cols) {
                ...
                const fontStyleStr = font.style === 'italic' ? 'italic' : 'normal'
                const fontWeight = font.weight === 'bold' ? 'bold' : 'normal'
                const fontSize = font.size ?? 13
                const fontFamily = font.family ?? 'system-ui, -apple-system, sans-serif'
                ctx.font = `${fontStyleStr} ${fontWeight} ${fontSize}px ${fontFamily}`
                ctx.fillStyle = font.color ?? '#1f2329'

                // 4b. 숫자 포맷 적용
                const displayText = style.numberFormat && rawValue !== null
                ...
                if (displayText) {
                    ...
                    if (decoration === 'underline' || decoration === 'line-through') {
                        ...
                        else if (textAlign === 'center') lineStartX = textX - textW / 2
