...
export function FilterDropdown({ col, uniqueValues, currentCriteria, position, onApply, onClose }: FilterDropdownProps) {
    const [tab, setTab] = React.useState<Tab>('values')

    // Values tab state
    const [selected, setSelected] = React.useState<Set<string>>(() => {
        if (currentCriteria?.type === 'values' && currentCriteria.selectedValues) {
    ...
    const [textOp, setTextOp] = React.useState<string>(
        currentCriteria?.type === 'text' ? (currentCriteria.textOperator ?? 'contains') : 'contains'
    )
    const [textVal, setTextVal] = React.useState(
        currentCriteria?.type === 'text' ? (currentCriteria.textValue ?? '') : ''
    )
    const [numOp, setNumOp] = React.useState<string>(
        currentCriteria?.type === 'number' ? (currentCriteria.numberOperator ?? 'equals') : 'equals'
    )
    const [numVal1, setNumVal1] = React.useState(
        currentCriteria?.type === 'number' ? String(currentCriteria.numberValue1 ?? '') : ''
    )
    const [numVal2, setNumVal2] = React.useState(
        currentCriteria?.type === 'number' ? String(currentCriteria.numberValue2 ?? '') : ''
    )
    ...
    return (
        <>
            ...
            >
                {/* 탭 헤더 */}
                <div style={TAB_BAR}>
                    <button style={TAB_BTN(tab === 'values')} onClick={() => setTab('values')}>
                        값 선택
                    </button>
                    <button style={TAB_BTN(tab === 'condition')} onClick={() => setTab('condition')}>
                        조건
                    </button>
                </div>
                ...
                {tab === 'condition' && (
                    <div style={{ padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                        <div style={{ display: 'flex', gap: 6 }}>
                            <button
                                style={{ ...BTN_SECONDARY, flex: 1, background: condType === 'text' ? '#e8f0fe' : '#fff', color: condType === 'text' ? '#1a73e8' : '#3c4043' }}
                                onClick={() => setCondType('text')}
                            >
                                텍스트
                            </button>
                            <button
                                style={{ ...BTN_SECONDARY, flex: 1, background: condType === 'number' ? '#e8f0fe' : '#fff', color: condType === 'number' ? '#1a73e8' : '#3c4043' }}
                        ...
                        {condType === 'text' && (
                            <>
                                <select
                                    ...
                                    style={{ height: 26, border: '1px solid #dadce0', borderRadius: 3, fontSize: 12, padding: '0 4px' }}
                                >
                                    {TEXT_OPS.map(op => (
                                ...
                                <input
                                    ...
                                    placeholder="값 입력"
                                    style={{ height: 26, border: '1px solid #dadce0', borderRadius: 3, fontSize: 12, padding: '0 6px', outline: 'none' }}
                                />
                            </>
                        )}
                ...
                {/* 하단 버튼 */}
                <div style={FOOTER}>
                    <button style={BTN_SECONDARY} onClick={onClose}>취소</button>
                    <button style={BTN_PRIMARY} onClick={handleApply}>확인</button>
                </div>
            </div>
        </>
    )
