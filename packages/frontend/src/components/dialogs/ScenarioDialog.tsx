...
export function ScenarioDialog({ sheetId, setCell, getCell, onClose }: Props) {
    ...
    const refresh = () =>
        setScenarios(scenarioManager.getScenariosForSheet(sheetId));
    ...
    const handleAdd = () => {
        ...
        for (let i = 0; i < cellRefs.length; i++) {
            ...
            if (!parsed) {
                setFormError(`셀 주소 "${ref}" 가 올바르지 않습니다.`);
                return;
            }
        ...
        if (changingCells.length === 0) {
            setFormError("변경 셀을 하나 이상 입력하세요.");
            return;
        }

        scenarioManager.addScenario(sheetId, {
            name: newName.trim(),
            changingCells,
            ...
            >
                {mode === "list" ? (
                    <>
                        ...
                        {scenarios.length === 0 ? (
                            <p style={{ color: "#5f6368", marginBottom: 12 }}>
                                시나리오가 없습니다.
                            </p>
                        ) : (
                            <ul
                            ...
                            >
                                {scenarios.map((s) => (
                                    ...
                                    >
                                        ...
                                        <button
                                            style={smallBtn("#1a73e8", "#fff")}
                                            onClick={() => handleShow(s.id)}
                                        >
                                            표시
                                        </button>
                                        <button
                                            style={smallBtn("#ea4335", "#fff")}
                            ...
                            >
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setCellRefs(arr);
                                        const cell = parseCellRef(
                                            e.target.value,
                                        );
                                        if (cell) {
                                            const existing = getCell(
                                                cell.row,
                                                cell.col,
                                            );
