...
function ChartItem({
    ...
    useEffect(() => {
        ...
        const instance = echarts.init(divRef.current, undefined, {
            renderer: "canvas",
        });
    ...
    useEffect(() => {
        instanceRef.current?.setOption(buildEChartsOption(chart, getCell), {
            notMerge: true,
        });
    ...
    const handleMouseDown = useCallback(
        (e: React.MouseEvent<HTMLDivElement>) => {
            ...
            const startCanvas = viewport.cellToPixel(
                chart.anchorRow,
                chart.anchorCol,
            );
            ...
            const onMove = (me: MouseEvent) => {
                ...
                const { row, col } = viewport.pixelToCell(
                    startCanvas.x + dx,
                    startCanvas.y + dy,
                );
        ...
        (e: React.MouseEvent) => {
            ...
            const onMove = (me: MouseEvent) => {
                ...
                for (let r = chart.anchorRow; r < chart.anchorRow + 100; r++) {
                    ...
                    if (accH >= targetH) {
                        heightRows = r - chart.anchorRow + 1;
                        break;
                    }
                }
