...
export function Toolbar() {
    ...
    React.useEffect(() => styleManager.subscribe(() => setStyleVersion((v) => v + 1)), [])
    ...
    const currentFontFamily = currentStyle.font?.family ?? 'system-ui, sans-serif'
    ...
    const getSelectedRanges = (): CellRange[] => {
        ...
        if (activeCell) {
            return [{
                start: { row: activeCell.row, col: activeCell.col, sheetId: activeSheetId },
                end: { row: activeCell.row, col: activeCell.col, sheetId: activeSheetId },
            }]
        }
        return []
    }
    ...
    return (
        ...
        >
            ...
            <Btn
                ...
                disabled={!canUndo}
                onClick={() => {
                    if (canUndo) document.dispatchEvent(
            ...
            <Btn
                ...
                disabled={!canRedo}
                onClick={() => {
                    if (canRedo) document.dispatchEvent(
            ...
            <select
                ...
                onChange={(e) => applyStyle({ font: { size: Number(e.target.value) } })}
            >
                {FONT_SIZES.map((s) => (
            ...
            <Btn
                ...
                active={isBold}
                onClick={() => applyStyle({ font: { weight: isBold ? 'normal' : 'bold' } })}
            />
            ...
            <Btn
                ...
                active={isItalic}
                onClick={() => applyStyle({ font: { style: isItalic ? 'normal' : 'italic' } })}
            />
            ...
            <Btn
                ...
                active={isUnderline}
                onClick={() => applyStyle({ font: { decoration: isUnderline ? 'none' : 'underline' } })}
            />
            ...
            <Btn
                ...
                active={isStrike}
                onClick={() => applyStyle({ font: { decoration: isStrike ? 'none' : 'line-through' } })}
            />
            ...
            <div style={{ position: 'relative', display: 'inline-flex' }}>
                ...
                >
                    <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                        <span style={{ fontSize: 12, lineHeight: 1, fontWeight: 700 }}>A</span>
                        <span style={{ width: 14, height: 3, background: currentFontColor, borderRadius: 1 }} />
                    </span>
                </button>
                <input
                    ref={fontColorRef}
                    ...
                    onChange={(e) => applyStyle({ font: { color: e.target.value } })}
                />
            </div>
            ...
            <div style={{ position: 'relative', display: 'inline-flex' }}>
                ...
                >
                    <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                        ...
                        <span style={{ width: 14, height: 3, background: currentBgColor, border: '1px solid #dadce0', borderRadius: 1 }} />
                    </span>
                </button>
                <input
                    ref={bgColorRef}
