# AGENTS.md — Cellix 개발 가이드

Codex가 이 프로젝트를 작업할 때 반드시 읽고 따라야 하는 기준 문서입니다.
...
이 섹션은 [multica-ai/andrej-karpathy-skills](https://github.com/multica-ai/andrej-karpathy-skills)의 `CLAUDE.md` 취지를 Cellix 작업 방식에 맞게 반영한 규칙입니다.
프로젝트별 규칙과 충돌할 경우 Cellix의 구체적인 기술 규칙을 우선하되, 아래 원칙을 모든 구현/리뷰/리팩터링의 기본 태도로 삼으세요.

### 0-1. 코딩 전 먼저 생각하기

- 요구사항을 임의로 해석하지 말고, 불확실한 부분은 가정으로 명시하거나 질문하세요.
- 해석이 여러 개라면 조용히 하나를 고르지 말고 가능한 선택지를 드러내세요.
- 더 단순하거나 안전한 접근이 있으면 먼저 제안하고, 필요하면 사용자 요청에도 근거 있게 이의를 제기하세요.
- 이해가 불충분한 상태에서 코드를 밀어붙이지 말고, 무엇이 애매한지 분명히 하세요.

### 0-2. 단순함 우선

- 요청된 문제를 해결하는 최소한의 코드를 작성하세요.
- 한 번만 쓰이는 코드에 추상화를 만들지 마세요.
- 요구되지 않은 확장성, 설정 옵션, 부가 기능을 추가하지 마세요.
- 실제로 발생 가능한 실패만 처리하고, 불가능한 경우를 위한 방어 코드로 흐름을 흐리지 마세요.
- 구현이 과하게 커졌다면 더 작은 해법으로 줄일 수 있는지 다시 검토하세요.

### 0-3. 외과적으로 수정하기

- 사용자 요청과 직접 연결되는 줄만 바꾸세요.
- 주변 코드, 주석, 포맷을 선의로 정리하거나 리팩터링하지 마세요.
- 기존 스타일이 마음에 들지 않아도 해당 파일의 패턴을 따르세요.
- 관련 없는 죽은 코드나 개선점을 발견하면 삭제하지 말고 사용자에게 알리세요.
- 내가 만든 변경으로 생긴 미사용 import, 변수, 함수만 정리하세요.

### 0-4. 검증 가능한 목표로 실행하기

- 작업을 시작할 때 성공 기준과 확인 방법을 구체화하세요.
- 버그 수정은 재현 가능한 실패 조건을 먼저 확인하고, 수정 후 같은 조건으로 검증하세요.
- 검증 명령이 있다면 실행하고 결과를 확인하세요. 실행하지 못했다면 이유와 남은 위험을 기록하세요.
- 큰 작업은 짧은 계획으로 쪼개되, 각 단계가 어떤 확인으로 완료되는지 분명히 하세요.
